...
def configure_password_generator(config, settings):
    ...
    if char_sum > settings.PASSWORD_LENGTH:
        raise ImproperlyConfigured(
            _('The sum of characters defined in password settings exceeds the value set in password_length setting')
        )
...
def configure_huey(config):
    ...
    if freq == 'daily':
        ...
        raise RuntimeError(
            _('Unknown value {freq} for task.scheduler_frequency in {path}').format(
                freq=freq,
...
def configure_ldap_auth(config, settings):
    ...
    start_tls = get_from_config(config, 'auth_ldap', 'start_tls', 'no').lower()
    if start_tls in {'1', 'enabled', 'true', 'yes'}:
        settings.AUTH_LDAP_START_TLS = True

    if get_from_config(config, 'auth_ldap', 'client_cert', None):
        settings.AUTH_LDAP_CONNECTION_OPTIONS[OPT_X_TLS_CERTFILE] = config.get('auth_ldap', 'client_cert')
        settings.AUTH_LDAP_CONNECTION_OPTIONS[OPT_X_TLS_KEYFILE] = config.get('auth_ldap', 'client_key')

    disable_cert_validation = get_from_config(
        config,
        'auth_ldap',
        'disable_server_cert_validation',
        'no',
    ).lower()
    if disable_cert_validation in {'1', 'enabled', 'true', 'yes'}:
        settings.AUTH_LDAP_CONNECTION_OPTIONS[OPT_X_TLS_REQUIRE_CERT] = OPT_X_TLS_NEVER
...
def configure_max_file_size(config, settings):
    settings.TEAMVAULT_MAX_FILE_SIZE = int(get_from_config(config, 'teamvault', 'max_file_size', '5242880'))
...
def configure_superuser_reads(config, settings):
    ...
    allow_reads = get_from_config(config, 'teamvault', 'allow_superuser_reads', 'False').lower()
    if allow_reads in {'1', 'enabled', 'true', 'yes'}:
...
def configure_whitenoise(settings):
    if not settings.DEBUG:
        settings.STORAGES = {'staticfiles': {'BACKEND': 'whitenoise.storage.StaticFilesStorage'}}
...
#revoke_unused_shares_after_days = 365  # task disabled if unset
    ...
    old_umask = umask(7)
    try:
        with pathlib.Path(filename).open('wt') as f:
...
def get_config():
    if not pathlib.Path(environ['TEAMVAULT_CONFIG_FILE']).is_file():
    ...
    pathlib.Path(environ['TEAMVAULT_CONFIG_FILE']).read_text()

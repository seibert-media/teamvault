...
def configure_ldap_auth(config, settings):
    ...
    if get_from_config(config, 'auth_ldap', 'start_tls', 'no').lower() in ('1', 'enabled', 'true', 'yes'):
...
def configure_logging(config):
    ...
    if get_from_config(config, 'teamvault', 'insecure_debug_mode', 'no').lower() in ('1', 'enabled', 'true', 'yes'):
        level = 'DEBUG'
...
def configure_superuser_reads(config, settings):
    ...
    if get_from_config(config, 'teamvault', 'allow_superuser_reads', 'False').lower() in (
        '1',
        'enabled',
        'true',
        'yes',

...
class CanonicalizeEntryUUIDTests(SimpleTestCase):
    def test_passes_through_string_uuid(self):
        self.assertEqual(
            canonicalize_ldap_uuid('5a02e51c-8d9e-4f5a-b8e1-2f3a4b5c6d7e'),
            '5a02e51c-8d9e-4f5a-b8e1-2f3a4b5c6d7e',
        )

    def test_decodes_utf8_bytes(self):
        self.assertEqual(
            canonicalize_ldap_uuid(b'5a02e51c-8d9e-4f5a-b8e1-2f3a4b5c6d7e'),
            '5a02e51c-8d9e-4f5a-b8e1-2f3a4b5c6d7e',
        )

    def test_canonicalizes_binary_guid_as_little_endian(self):
        self.assertEqual(canonicalize_ldap_uuid(GUID.bytes_le), str(GUID))

    def test_rejects_undecodable_bytes(self):
        self.assertIsNone(canonicalize_ldap_uuid(b'\xff\xfe\xfd'))

    def test_rejects_overlong_value(self):
        self.assertIsNone(canonicalize_ldap_uuid('x' * 37))

    def test_rejects_empty_value(self):
        self.assertIsNone(canonicalize_ldap_uuid(''))
        self.assertIsNone(canonicalize_ldap_uuid(b''))
        self.assertIsNone(canonicalize_ldap_uuid(None))

    def test_passes_through_non_uuid_string_id(self):
        self.assertEqual(canonicalize_ldap_uuid('some-opaque-id'), 'some-opaque-id')


class EntryUUIDFilterTermTests(SimpleTestCase):
    def test_uuid_value_matches_string_and_binary_forms(self):
        term = ldap_uuid_filter_term('objectGUID', str(GUID))
    ...
    def test_non_uuid_value_gets_single_escaped_term(self):
        self.assertEqual(ldap_uuid_filter_term('entryUUID', 'a(b)c'), r'(entryUUID=a\28b\29c)')
